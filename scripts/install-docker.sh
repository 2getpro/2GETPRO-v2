...
check_system_requirements() {
    ...
    echo -e "   ${CYAN}RAM:${NC} ${WHITE}${total_ram}GB${NC} ${YELLOW}(минимум: 1GB, рекомендуется: 4GB)${NC}"
    
    if [[ $total_ram -lt 1 ]]; then
        print_warning "Очень мало RAM, бот может не запуститься"
    elif [[ $total_ram -lt 2 ]]; then
        print_warning "Мало RAM, возможны проблемы с производительностью"
    elif [[ $total_ram -lt 4 ]]; then
        print_info "RAM достаточно для базовой работы"
    else
        print_success "RAM достаточно для стабильной работы"
    fi
    
    # Проверка свободного места на диске
    local free_space=$(df -BG / | awk 'NR==2 {print $4}' | sed 's/G//')
    echo -e "   ${CYAN}Свободное место:${NC} ${WHITE}${free_space}GB${NC} ${YELLOW}(минимум: 5GB, рекомендуется: 20GB)${NC}"
    
    if [[ $free_space -lt 5 ]]; then
        print_warning "Очень мало места, может не хватить для Docker образов"
    elif [[ $free_space -lt 10 ]]; then
        print_warning "Мало места, рекомендуется освободить диск"
    elif [[ $free_space -lt 20 ]]; then
        print_info "Места достаточно для базовой работы"
    else
        print_success "Места достаточно для стабильной работы"
    fi
    
    # Проверка подключения к интернету
    if ! ping -c 1 8.8.8.8 &> /dev/null; then
        print_error "Нет подключения к интернету"
        print_info "Подключение к интернету необходимо для установки"
        exit 1
    fi
    print_success "Подключение к интернету активно"
    
    echo
    print_info "💡 Бот может работать на минимальных ресурсах (1 CPU, 1GB RAM)"
    print_info "💡 Для стабильной работы рекомендуется: 2 CPU, 4GB RAM, 20GB диск"
    echo
}
