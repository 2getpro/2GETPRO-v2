# 📜 Скрипты управления 2GETPRO v2

Этот каталог содержит автоматизированные скрипты для установки, обновления и удаления Telegram-бота 2GETPRO v2 на Ubuntu 24.04.03 LTS.

---

## 📋 Содержание

- [Обзор скриптов](#обзор-скриптов)
- [Установка](#установка)
- [Обновление](#обновление)
- [Удаление](#удаление)
- [Решение проблем](#решение-проблем)
- [Часто задаваемые вопросы](#часто-задаваемые-вопросы)

---

## 🔧 Обзор скриптов

### [`install.sh`](install.sh:1) - Автоматический установщик

Полностью автоматизирует процесс установки бота на чистую систему Ubuntu 24.04.03 LTS.

**Что делает:**
- ✅ Проверяет системные требования
- ✅ Устанавливает все необходимые компоненты (Python, PostgreSQL, Redis, Git)
- ✅ Создает и настраивает базу данных
- ✅ Клонирует репозиторий и устанавливает зависимости
- ✅ Генерирует конфигурационный файл
- ✅ Настраивает systemd сервис
- ✅ Применяет миграции базы данных
- ✅ Опционально настраивает веб-хуки с SSL

### [`update.sh`](update.sh:1) - Скрипт обновления

Безопасно обновляет бота до последней версии с автоматическим резервным копированием.

**Что делает:**
- ✅ Создает резервную копию перед обновлением
- ✅ Обновляет код из git-репозитория
- ✅ Обновляет Python зависимости
- ✅ Применяет новые миграции базы данных
- ✅ Обновляет systemd сервис и скрипты
- ✅ Проверяет работоспособность после обновления
- ✅ Поддерживает откат в случае проблем

### [`uninstall.sh`](uninstall.sh:1) - Скрипт удаления

Полностью удаляет бота с системы с возможностью сохранения данных.

**Что делает:**
- ✅ Останавливает и удаляет systemd сервис
- ✅ Удаляет файлы проекта
- ✅ Опционально удаляет базу данных
- ✅ Опционально удаляет системного пользователя
- ✅ Опционально удаляет логи
- ✅ Создает резервную копию конфигурации

---

## 🚀 Установка

### Базовая установка (интерактивная)

```bash
# Скачайте скрипт или клонируйте репозиторий
cd /tmp
git clone https://github.com/your-org/2GETPRO_v2.git
cd 2GETPRO_v2/scripts

# Сделайте скрипт исполняемым
chmod +x install.sh

# Запустите установку
sudo ./install.sh
```

Скрипт задаст вам следующие вопросы:
1. **Токен Telegram бота** - получите у [@BotFather](https://t.me/BotFather)
2. **ID администраторов** - узнайте у [@userinfobot](https://t.me/userinfobot)
3. **Параметры базы данных** - имя БД, пользователь, пароль
4. **URL и ключ API панели VPN** - данные вашей панели управления
5. **Настройки платежных систем** - YooKassa, CryptoPay, Stars
6. **Настройки веб-хуков** (опционально) - домен и email для SSL

### Тихая установка

Для автоматической установки без интерактивных запросов:

```bash
sudo ./install.sh --silent --yes
```

⚠️ **Внимание:** В тихом режиме используются значения по умолчанию. Убедитесь, что они вам подходят.

### Проверка без изменений (Dry Run)

Проверьте, что будет сделано, без реальных изменений:

```bash
sudo ./install.sh --dry-run
```

### Параметры командной строки

```bash
sudo ./install.sh [ОПЦИИ]

Опции:
  -h, --help              Показать справку
  -s, --silent            Тихий режим (без интерактивных запросов)
  -y, --yes               Автоматически отвечать "да" на все вопросы
  -d, --dry-run           Режим проверки без реальных изменений
  --skip-system-update    Пропустить обновление системы
  --skip-webhooks         Пропустить настройку веб-хуков
  --uninstall             Удалить установленный бот
```

### Примеры использования

```bash
# Стандартная интерактивная установка
sudo ./install.sh

# Быстрая установка с автоподтверждением
sudo ./install.sh --yes

# Установка без обновления системы
sudo ./install.sh --skip-system-update

# Проверка перед установкой
sudo ./install.sh --dry-run
```

---

## 🔄 Обновление

### Стандартное обновление

```bash
cd /opt/2getpro-v2/scripts
sudo ./update.sh
```

Скрипт автоматически:
1. Создаст резервную копию
2. Остановит бота
3. Обновит код и зависимости
4. Применит миграции
5. Запустит бота
6. Проверит работоспособность

### Проверка наличия обновлений

Проверьте, доступны ли обновления, без их установки:

```bash
sudo ./update.sh --check-only
```

### Обновление без резервной копии

⚠️ **Не рекомендуется для продакшена!**

```bash
sudo ./update.sh --skip-backup
```

### Обновление из другой ветки

```bash
sudo ./update.sh --branch develop
```

### Обновление без автоматического перезапуска

```bash
sudo ./update.sh --no-restart

# Затем запустите вручную
sudo systemctl start 2getpro-v2
```

### Параметры командной строки

```bash
sudo ./update.sh [ОПЦИИ]

Опции:
  -h, --help              Показать справку
  -f, --force             Принудительное обновление
  -b, --branch BRANCH     Обновить из указанной ветки (по умолчанию: main)
  --skip-backup           Не создавать резервную копию
  --skip-migrations       Не применять миграции
  --no-restart            Не перезапускать сервис автоматически
  --check-only            Только проверить наличие обновлений
```

### Примеры использования

```bash
# Стандартное обновление
sudo ./update.sh

# Проверка обновлений
sudo ./update.sh --check-only

# Обновление из ветки develop
sudo ./update.sh --branch develop

# Принудительное обновление без перезапуска
sudo ./update.sh --force --no-restart

# Обновление без резервной копии (быстрое)
sudo ./update.sh --skip-backup
```

### Откат обновления

Если после обновления возникли проблемы:

#### Автоматический откат

```bash
cd /opt/2getpro-v2
sudo systemctl stop 2getpro-v2
sudo -u 2getpro git reset --hard HEAD~1
sudo systemctl start 2getpro-v2
```

#### Восстановление из резервной копии

```bash
# Найдите последнюю резервную копию
ls -lt /opt/2getpro-backups/

# Распакуйте резервную копию
cd /opt/2getpro-backups
tar -xzf 2getpro-backup-YYYYMMDD_HHMMSS.tar.gz

# Восстановите конфигурацию
sudo cp 2getpro-backup-YYYYMMDD_HHMMSS/.env.production /opt/2getpro-v2/

# Восстановите базу данных (если нужно)
sudo -u postgres psql 2getpro_v2_db < 2getpro-backup-YYYYMMDD_HHMMSS/database.sql

# Перезапустите бота
sudo systemctl restart 2getpro-v2
```

---

## 🗑️ Удаление

### Базовое удаление

Удаляет файлы проекта и сервис, но сохраняет базу данных и пользователя:

```bash
cd /opt/2getpro-v2/scripts
sudo ./uninstall.sh
```

### Полное удаление

Удаляет всё, включая базу данных, пользователя и логи:

```bash
sudo ./uninstall.sh --full --force
```

⚠️ **Внимание:** Все данные будут потеряны безвозвратно!

### Удаление с сохранением базы данных

```bash
sudo ./uninstall.sh --remove-user --remove-logs
```

### Параметры командной строки

```bash
sudo ./uninstall.sh [ОПЦИИ]

Опции:
  -h, --help              Показать справку
  -f, --force             Принудительное удаление без подтверждений
  --remove-database       Удалить базу данных PostgreSQL
  --remove-user           Удалить системного пользователя
  --remove-logs           Удалить все логи
  --no-backup             Не создавать резервную копию конфигурации
  --full                  Полное удаление (все опции выше)
```

### Примеры использования

```bash
# Базовое удаление с подтверждениями
sudo ./uninstall.sh

# Полное удаление без подтверждений
sudo ./uninstall.sh --full --force

# Удаление с сохранением базы данных
sudo ./uninstall.sh --remove-user --remove-logs

# Удаление только файлов проекта
sudo ./uninstall.sh --force
```

### Что остается после удаления

По умолчанию сохраняются:
- ✅ База данных PostgreSQL
- ✅ Системный пользователь `2getpro`
- ✅ Логи в `/var/log/2getpro`
- ✅ Резервная копия конфигурации в `/opt/2getpro-backups`
- ✅ Установленные системные пакеты (PostgreSQL, Redis, Nginx)

Для полного удаления используйте опцию `--full`.

---

## 🔍 Решение проблем

### Проблема: Скрипт не запускается

**Ошибка:** `Permission denied`

**Решение:**
```bash
# Сделайте скрипт исполняемым
chmod +x install.sh update.sh uninstall.sh

# Запустите с правами root
sudo ./install.sh
```

---

### Проблема: Ошибка при установке зависимостей

**Ошибка:** `ModuleNotFoundError` или ошибки pip

**Решение:**
```bash
# Обновите pip
sudo -u 2getpro bash -c "
  cd /opt/2getpro-v2
  source venv/bin/activate
  pip install --upgrade pip setuptools wheel
  pip install -r requirements.txt --force-reinstall
"
```

---

### Проблема: База данных не создается

**Ошибка:** `could not connect to server`

**Решение:**
```bash
# Проверьте статус PostgreSQL
sudo systemctl status postgresql

# Перезапустите PostgreSQL
sudo systemctl restart postgresql

# Проверьте подключение
sudo -u postgres psql -l
```

---

### Проблема: Бот не запускается после установки

**Решение:**
```bash
# Проверьте логи
sudo journalctl -u 2getpro-v2 -n 50

# Проверьте конфигурацию
sudo cat /opt/2getpro-v2/.env.production

# Проверьте права доступа
sudo ls -la /opt/2getpro-v2/

# Попробуйте запустить вручную
sudo -u 2getpro bash -c "
  cd /opt/2getpro-v2
  source venv/bin/activate
  python main.py
"
```

---

### Проблема: Обновление не применяется

**Решение:**
```bash
# Проверьте статус git
cd /opt/2getpro-v2
sudo -u 2getpro git status

# Сбросьте локальные изменения
sudo -u 2getpro git reset --hard HEAD
sudo -u 2getpro git clean -fd

# Попробуйте обновить снова
sudo ./scripts/update.sh --force
```

---

### Проблема: Ошибки при применении миграций

**Решение:**
```bash
# Проверьте подключение к БД
sudo -u postgres psql -U 2getpro_user -d 2getpro_v2_db -h localhost

# Примените миграции вручную
sudo -u 2getpro bash -c "
  cd /opt/2getpro-v2
  source venv/bin/activate
  python db/migrator.py
"

# Проверьте логи миграций
sudo cat /var/log/2getpro-update.log
```

---

### Проблема: Веб-хуки не работают

**Решение:**
```bash
# Проверьте Nginx
sudo nginx -t
sudo systemctl status nginx

# Проверьте SSL сертификат
sudo certbot certificates

# Проверьте логи Nginx
sudo tail -f /var/log/nginx/error.log

# Проверьте, что порт 8080 открыт
sudo netstat -tulpn | grep 8080
```

---

## ❓ Часто задаваемые вопросы

### Можно ли установить бота на другую версию Ubuntu?

Скрипты оптимизированы для Ubuntu 24.04.03 LTS, но должны работать на Ubuntu 22.04 и 20.04. Для других дистрибутивов может потребоваться модификация.

### Как изменить конфигурацию после установки?

```bash
# Отредактируйте файл конфигурации
sudo nano /opt/2getpro-v2/.env.production

# Перезапустите бота
sudo systemctl restart 2getpro-v2
```

### Как посмотреть логи бота?

```bash
# Логи в реальном времени
sudo journalctl -u 2getpro-v2 -f

# Последние 100 строк
sudo journalctl -u 2getpro-v2 -n 100

# Логи за сегодня
sudo journalctl -u 2getpro-v2 --since today

# Логи установки/обновления
sudo cat /var/log/2getpro-install.log
sudo cat /var/log/2getpro-update.log
```

### Как создать резервную копию вручную?

```bash
# Создайте директорию для бэкапа
sudo mkdir -p /opt/2getpro-backups/manual-backup-$(date +%Y%m%d)

# Скопируйте конфигурацию
sudo cp /opt/2getpro-v2/.env.production /opt/2getpro-backups/manual-backup-$(date +%Y%m%d)/

# Создайте дамп базы данных
sudo -u postgres pg_dump 2getpro_v2_db > /opt/2getpro-backups/manual-backup-$(date +%Y%m%d)/database.sql

# Сожмите резервную копию
cd /opt/2getpro-backups
sudo tar -czf manual-backup-$(date +%Y%m%d).tar.gz manual-backup-$(date +%Y%m%d)
sudo rm -rf manual-backup-$(date +%Y%m%d)
```

### Как перенести бота на другой сервер?

1. **На старом сервере:**
```bash
# Создайте резервную копию
sudo ./scripts/update.sh --skip-migrations
cd /opt/2getpro-backups
sudo tar -czf full-backup.tar.gz 2getpro-backup-*

# Скопируйте на новый сервер
scp full-backup.tar.gz user@new-server:/tmp/
```

2. **На новом сервере:**
```bash
# Установите бота
sudo ./scripts/install.sh

# Восстановите конфигурацию
cd /tmp
tar -xzf full-backup.tar.gz
sudo cp 2getpro-backup-*/. env.production /opt/2getpro-v2/

# Восстановите базу данных
sudo -u postgres psql 2getpro_v2_db < 2getpro-backup-*/database.sql

# Перезапустите бота
sudo systemctl restart 2getpro-v2
```

### Как обновить только зависимости без обновления кода?

```bash
sudo -u 2getpro bash -c "
  cd /opt/2getpro-v2
  source venv/bin/activate
  pip install -r requirements.txt --upgrade
"
sudo systemctl restart 2getpro-v2
```

### Как запустить бота в режиме отладки?

```bash
# Остановите сервис
sudo systemctl stop 2getpro-v2

# Запустите вручную
sudo -u 2getpro bash -c "
  cd /opt/2getpro-v2
  source venv/bin/activate
  python main.py
"

# Для выхода нажмите Ctrl+C
# Затем запустите сервис снова
sudo systemctl start 2getpro-v2
```

### Где хранятся резервные копии?

Все резервные копии хранятся в `/opt/2getpro-backups/`:
- Автоматические копии при обновлении
- Копии при удалении (если не отключено)
- Ручные резервные копии

Скрипт автоматически удаляет старые копии, оставляя последние 5.

---

## 📞 Получение помощи

Если вы столкнулись с проблемой:

1. **Проверьте логи:**
   ```bash
   sudo journalctl -u 2getpro-v2 -n 100
   sudo cat /var/log/2getpro-install.log
   ```

2. **Проверьте документацию:**
   - [Руководство по установке](../docs/deployment/INSTALLATION_GUIDE.md)
   - [Решение проблем](../docs/operations/troubleshooting.md)

3. **Создайте issue на GitHub:**
   - Опишите проблему
   - Приложите логи
   - Укажите версию Ubuntu и бота

4. **Обратитесь в поддержку:**
   - Telegram: @your_support
   - Email: support@example.com

---

## 📝 Примечания

- Все скрипты требуют прав root (sudo)
- Скрипты логируют все действия в `/var/log/`
- Резервные копии создаются автоматически
- Скрипты идемпотентны (можно запускать повторно)
- Поддерживается откат изменений при ошибках

---

## 🔐 Безопасность

- Файлы конфигурации защищены правами 600
- Пароли генерируются автоматически
- Резервные копии содержат чувствительные данные
- Используйте HTTPS для веб-хуков
- Регулярно обновляйте систему и зависимости

---

## 📚 Дополнительные ресурсы

- [Документация по API](../docs/api/README.md)
- [Руководство по Docker](../docs/deployment/docker.md)
- [Руководство по Kubernetes](../docs/deployment/kubernetes.md)
- [Руководство по продакшену](../docs/deployment/production.md)

---

**Версия документа:** 1.0  
**Дата обновления:** 2024-11-27  
**Автор:** 2GETPRO Team